...
from ghrp.model_definitions.def_net_distillation import NNmoduleARD
...
class NN_tune_trainable_from_checkpoint(Trainable):
    def setup(self, config):
        ...
        with torch.no_grad():
            loss_test_prev, acc_test_prev = nnmodule_previous.test_epoch(
                self.testloader, 0
            )
        assert torch.isclose(
            torch.tensor(loss_test), torch.tensor(loss_test_prev), 1e-3
        ), f"original test loss: {loss_test_prev}, new test loss {loss_test}"
        assert torch.isclose(
            torch.tensor(acc_test), torch.tensor(acc_test_prev), 1e-2
        ), f"original test loss: {acc_test_prev}, new test loss {acc_test}"
    ...
    def step(self):
        ...
        if self._iteration < 0:
            ...
            loss_train, acc_train = self.NN.test_epoch(
                self.trainloader, self._iteration
            )

        else:
            loss_train, acc_train = self.NN.train_epoch(
                self.trainloader, self._iteration, idx_out=10
            )
        # run one test epoch
